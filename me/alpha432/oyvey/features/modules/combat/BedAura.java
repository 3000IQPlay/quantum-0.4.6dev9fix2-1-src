...
extends Module {
    ...
    public void onEnable() {
        if (!BedAura.fullNullCheck() && this.shouldServer()) {
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (BedAura.fullNullCheck() || BedAura.mc.player.dimension != -1 && BedAura.mc.player.dimension != 1 || this.shouldServer()) {
            return;
        }
        if (event.getStage() == 0) {
            this.doBedBomb();
    ...
    private void mapBeds() {
        ...
        if (this.removeTiles.getValue().booleanValue()) {
            ...
            for (BedData data : removedBlocks) {
                ...
                if (!(BedAura.mc.player.getDistanceSq(blockPos) <= MathUtil.square(this.breakRange.getValue().floatValue())) || !((double)(selfDamage = DamageUtil.calculateDamage(pos, (Entity)BedAura.mc.player)) + 1.0 < (double)EntityUtil.getHealth((Entity)BedAura.mc.player)) && DamageUtil.canTakeDamage(this.suicide.getValue())) continue;
    ...
    public void craftTable() {
        ...
        if (woodSlot != -1) {
            ...
            if (table != -1) {
                ...
                BedAura.mc.playerController.windowClick(0, this.tableSlot.getValue().intValue(), 0, ClickType.PICKUP, (EntityPlayer)BedAura.mc.player);
                BedAura.mc.playerController.windowClick(0, table, 0, ClickType.PICKUP, (EntityPlayer)BedAura.mc.player);
            }
        }
    }
