...
extends Module {
    ...
    public NametagsC() {
        super("Nametags", "ntc", Module.Category.RENDER, true, false, true);
        INSTANCE = this;
    }

    public static NametagsC getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new NametagsC();
        }
        return INSTANCE;
    }
    ...
    public void onRender3D(Render3DEvent event) {
        if (!NametagsC.fullNullCheck()) {
            for (EntityPlayer player : NametagsC.mc.world.playerEntities) {
                if (player == null || player.equals((Object)NametagsC.mc.player) || !player.isEntityAlive() || player.isInvisible() && !this.invisibles.getValue().booleanValue() || this.onlyFov.getValue().booleanValue() && !RotationUtil.isInFov((Entity)player)) continue;
                double x = this.interpolate(player.lastTickPosX, player.posX, event.getPartialTicks()) - NametagsC.mc.getRenderManager().renderPosX;
                double y = this.interpolate(player.lastTickPosY, player.posY, event.getPartialTicks()) - NametagsC.mc.getRenderManager().renderPosY;
                double z = this.interpolate(player.lastTickPosZ, player.posZ, event.getPartialTicks()) - NametagsC.mc.getRenderManager().renderPosZ;
    ...
    private void renderProperNameTag(EntityPlayer player, double x, double y, double z, float delta) {
        ...
        double distance = camera.getDistance(x + NametagsC.mc.getRenderManager().viewerPosX, y + NametagsC.mc.getRenderManager().viewerPosY, z + NametagsC.mc.getRenderManager().viewerPosZ);
        ...
        GlStateManager.rotate((float)(-NametagsC.mc.getRenderManager().playerViewY), (float)0.0f, (float)1.0f, (float)0.0f);
        GlStateManager.rotate((float)NametagsC.mc.getRenderManager().playerViewX, (float)(NametagsC.mc.gameSettings.thirdPersonView == 2 ? -1.0f : 1.0f), (float)0.0f, (float)0.0f);
        GlStateManager.scale((double)(-scale), (double)(-scale), (double)scale);
        GlStateManager.disableDepth();
        GlStateManager.enableBlend();
        GlStateManager.disableAlpha();
        if (this.rect.getValue().booleanValue()) {
            this.drawRect(-width - 2, -(NametagsC.mc.fontRenderer.FONT_HEIGHT + 1), (float)width + 2.0f, 1.5f, 0x55000000);
            if (this.outline.getValue().booleanValue()) {
                int color = this.colorSync.getValue() != false ? ClickGui.getInstance().getColor() : new Color(this.redSetting.getValue(), this.greenSetting.getValue(), this.blueSetting.getValue(), this.alphaSetting.getValue()).getRGB();
                this.drawOutlineRect(-width - 2, -(NametagsC.mc.fontRenderer.FONT_HEIGHT + 1), (float)width + 2.0f, 1.5f, color);
            }
        }
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float partialTicks) {
        ...
        double distance = camera.getDistance(x + NametagsC.mc.getRenderManager().viewerPosX, y + NametagsC.mc.getRenderManager().viewerPosY, z + NametagsC.mc.getRenderManager().viewerPosZ);
        int width = NametagsC.mc.fontRenderer.getStringWidth(this.getDisplayTag(player)) / 2;
        ...
        GlStateManager.translate((float)((float)x), (float)((float)tempY + 1.4f), (float)((float)z));
        GlStateManager.rotate((float)(-NametagsC.mc.getRenderManager().playerViewY), (float)0.0f, (float)1.0f, (float)0.0f);
        float thirdPersonOffset = NametagsC.mc.gameSettings.thirdPersonView == 2 ? -1.0f : 1.0f;
        GlStateManager.rotate((float)NametagsC.mc.getRenderManager().playerViewX, (float)thirdPersonOffset, (float)0.0f, (float)0.0f);
        GlStateManager.scale((double)(-scale), (double)(-scale), (double)scale);
        GlStateManager.disableDepth();
        GlStateManager.enableBlend();
        GlStateManager.disableAlpha();
    ...
    private void renderItemStack(EntityPlayer player, ItemStack stack, int x, int y, boolean item) {
        ...
        if (item) {
            ...
            mc.getRenderItem().renderItemOverlays(NametagsC.mc.fontRenderer, stack, x, y);
        }
    ...
    private void renderEnchantmentText(EntityPlayer player, ItemStack stack, int x, int y) {
        ...
        for (int index = 0; index < enchants.tagCount(); ++index) {
            ...
            String string2 = enc.isCurse() ? (this.altEnchantNames.getValue().booleanValue() ? enc.getTranslatedName((int)level).substring(11).substring(0, this.altEnchantNames.getValue() != false ? 3 : 1) : TextFormatting.RED + enc.getTranslatedName((int)level).substring(11).substring(0, this.altEnchantNames.getValue().booleanValue() ? (enc.getMaxLevel() == 1 ? 3 : 2) : 1)) : (string = (encName = enc.getTranslatedName((int)level).substring(0, this.altEnchantNames.getValue().booleanValue() ? (enc.getMaxLevel() == 1 ? 3 : 2) : 1)));
