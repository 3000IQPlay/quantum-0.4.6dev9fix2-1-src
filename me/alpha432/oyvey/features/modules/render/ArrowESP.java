...
extends Module {
    ...
    private final Setting<Boolean> rainbow = this.register(new Setting<Boolean>("NiggerTest", false));
        ...
        ArrowESP.mc.world.loadedEntityList.forEach(o -> {
            if (o instanceof EntityPlayer && this.isValid((EntityPlayer)o)) {
                ...
                if (pos != null && !this.isOnScreen(pos) && !RenderUtil.isInViewFrustrum((Entity)entity)) {
                    Color color = this.rainbow.getValue() != false ? new Color(OyVey.INSTANCE.getColorManager().getColor().getRed(), OyVey.INSTANCE.getColorManager().getColor().getGreen(), OyVey.INSTANCE.getColorManager().getColor().getBlue(), (int)MathHelper.clamp((float)(255.0f - 255.0f / (float)this.fadeDistance.getValue().intValue() * ArrowESP.mc.player.getDistance((Entity)entity)), (float)100.0f, (float)255.0f)) : EntityUtil.getColor((Entity)entity, this.red.getValue(), this.green.getValue(), this.blue.getValue(), (int)MathHelper.clamp((float)(255.0f - 255.0f / (float)this.fadeDistance.getValue().intValue() * ArrowESP.mc.player.getDistance((Entity)entity)), (float)100.0f, (float)255.0f), true);
                    ...
                    float yaw = this.getRotations((EntityLivingBase)entity) - ArrowESP.mc.player.rotationYaw;
    ...
    private boolean isOnScreen(Vec3d pos) {
        ...
        int n4 = n = ArrowESP.mc.gameSettings.guiScale == 0 ? 1 : ArrowESP.mc.gameSettings.guiScale;
        if (!(pos.x / (double)n >= 0.0)) {
            return false;
        }
        int n5 = n2 = ArrowESP.mc.gameSettings.guiScale == 0 ? 1 : ArrowESP.mc.gameSettings.guiScale;
        if (!(pos.x / (double)n2 <= (double)Display.getWidth())) {
            return false;
        }
        int n6 = n3 = ArrowESP.mc.gameSettings.guiScale == 0 ? 1 : ArrowESP.mc.gameSettings.guiScale;
        if (!(pos.y / (double)n3 >= 0.0)) {
            return false;
        }
        int n42 = ArrowESP.mc.gameSettings.guiScale == 0 ? 1 : ArrowESP.mc.gameSettings.guiScale;
