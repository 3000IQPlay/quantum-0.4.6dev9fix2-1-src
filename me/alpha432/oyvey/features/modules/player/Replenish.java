...
extends Module {
    ...
    private final Setting<Integer> gapStack = this.register(new Setting<Integer>("GapStack", 1, 1, 64));
    private final Setting<Integer> xpStackAt = this.register(new Setting<Integer>("XPStack", 1, 1, 64));
    private final Setting<Integer> webStack = this.register(new Setting<Integer>("WebStack", 1, 1, 64));
    private final Timer timer = new Timer();
    private final ArrayList<Item> Hotbar = new ArrayList();

    public Replenish() {
        super("Refill", "rf", Module.Category.PLAYER, false, false, false);
    }
    ...
    private boolean RefillSlotIfNeed(int p_Slot) {
        ...
        if (l_Stack.getItem().equals(Blocks.WEB) && l_Stack.getCount() > this.webStack.getValue()) {
            return false;
        }
