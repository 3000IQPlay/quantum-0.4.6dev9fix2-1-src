...
extends Module {
    ...
    private final Setting<Boolean> inwater = this.register(new Setting<Boolean>("AntiWater", true));
    ...
    private final Setting<Integer> yOffset = this.register(new Setting<Integer>("HopHeight", 400, 350, 500));
    ...
    private final Setting<Integer> step = this.register(new Setting<Object>("SetStep", Integer.valueOf(1), Integer.valueOf(0), Integer.valueOf(2), v -> this.mode.getValue() == Mode.CWalk));
    ...
    public void onMove(MoveEvent event) {
        ...
        if (Strafe.mc.player.isInWater() && this.inwater.getValue().booleanValue()) {
            return;
        }
        if (!Strafe.mc.player.onGround) {
        ...
        if (this.mode.getValue() == Mode.NCP) {
            this.doNCP(event);
        } else if (this.mode.getValue() == Mode.CWalk) {
            ...
            if (this.limiter2.getValue().booleanValue() && Strafe.mc.player.onGround && OyVey.speedManager.getSpeedKpH() < (double)this.speedLimit2.getValue().floatValue()) {
                this.stage = 2;
            }
            ...
            if (this.stage == 1 && EntityUtil.isMoving()) {
                ...
                if (Strafe.mc.world.getCollisionBoxes((Entity)Strafe.mc.player, Strafe.mc.player.getEntityBoundingBox().offset(0.0, Strafe.mc.player.motionY, 0.0)).size() > 0 || Strafe.mc.player.collidedVertically && this.stage > 0) {
                    this.stage = this.bhop2.getValue() != false && OyVey.speedManager.getSpeedKpH() >= (double)this.speedLimit.getValue().floatValue() ? 0 : (Strafe.mc.player.moveForward != 0.0f || Strafe.mc.player.moveStrafing != 0.0f ? 1 : 0);
                }
    ...
    private boolean shouldReturn() {
        return OyVey.moduleManager.isModuleEnabled("Freecam") || OyVey.moduleManager.isModuleEnabled("Packetfly") || OyVey.moduleManager.isModuleEnabled("ElytraFlight") || OyVey.moduleManager.isModuleEnabled("Phase");
    }

    @SubscribeEvent
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketPlayerPosLook && this.noLag.getValue().booleanValue()) {
            this.stage = this.mode.getValue() == Mode.CWalk && (this.limiter2.getValue() != false || this.bhop2.getValue() != false) ? 1 : 4;
        }
    }
